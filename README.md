...
程式架構
	CHEMICAL_DATABASE: 字典結構，儲存物質名稱、類型與解離常數。
	class UniversalTitrator:__init__: 處理 K_a / K_b 的轉換與物種標籤生成。
	get_alphas: 計算分佈係數 
	solve_ph: 核心計算邏輯，整合體積變化並呼叫 brentq 求解。
	main(): 負責處理使用者介面、繪圖邏輯與結果輸出
